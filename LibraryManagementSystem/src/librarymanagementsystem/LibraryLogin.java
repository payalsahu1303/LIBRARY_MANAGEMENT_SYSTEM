...
public class LibraryLogin {
    ...
    public static void viewBooks() {
        ...
        viewBooksFrame.setLocationRelativeTo(null); // Center the window on the screen
    ...
    public static void issueBook() {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASSWORD);
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, Integer.parseInt(pnr)); // Assuming PNR corresponds to user_id
            stmt.setInt(2, Integer.parseInt(bookId));
            stmt.setDate(3, new java.sql.Date(System.currentTimeMillis())); // Current date
            stmt.setDate(4, new java.sql.Date(System.currentTimeMillis() + 604800000L)); // Due date (1 week from now)
            stmt.executeUpdate();
    ...
    public static void returnBook() {
        ...
        try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASSWORD);
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setDate(1, new java.sql.Date(System.currentTimeMillis())); // Current date
            stmt.setDouble(2, 0.0); // Assuming no fine for simplicity
